...
#Emotion Timeline: Create a timeline or graph that shows the changes in detected emotions over time while the user is 
#interacting with the webcam.

# 2d array to include timestamps and emotion which will be used to plot the emotion timeline
# research dynamic DNS. 
# Mention about the ethics in your IPR
...
# Create and display the Emotion Timeline graph
plt.figure(figsize=(10, 5))
plt.plot(emotion_timeline)
plt.xlabel(' ')
plt.ylabel('Detected Emotion')
plt.title('Emotion Timeline')
plt.xticks(range(len(emotion_timeline)))
plt.yticks(range(len(DeepFace.get_supported_emotions())), list(DeepFace.get_supported_emotions().keys()))
plt.grid(True)
plt.show()
